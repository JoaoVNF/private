...
//=============================================================
 class HeleShawEquations : public virtual FiniteElement
 {
  ...
  /// Constructor
   HeleShawEquations() : Upper_wall_fct_pt(0), Wall_speed_fct_pt(0),
                                           ...
                                           Vector<double>& dbdx) const
    {
     ...
     if (Upper_wall_fct_pt==0)
     ...
     else
      {
       ...
       (*Upper_wall_fct_pt)(x,b,dbdt,dbdx);
      }
    }
      ...
      if(local_eqn >= 0)
       {
        ...
        // context.
        residuals[local_eqn] += dbdt*test(l)*W;

        // The HeleShaw bit itself
        for(unsigned k=0; k<2; k++)
         {
          residuals[local_eqn] += pow(b,3.0)*interpolated_dpdx[k]*
           dtestdx(l,k)*W;

          // obacht moving frame terms
          residuals[local_eqn] -= U_wall[k]*dbdx[k]*test(l)*W;
         }
              ...
              for(unsigned i=0; i<2; i++)
               {
                jacobian(local_eqn,local_unknown)
                 += pow(b,3)*dpsidx(l2,i)*dtestdx(l,i)*W;
                // no contribution from the moving frame term 
                // (unless wall velocity and/or h is an unknown)
               }
             }
           }
         }
