...
namespace Problem_Parameter
{
 ...
 double h_sheet = 0.34*1.0e-3; //0.34*1.0e-3; //3*1e-5;
 ...
 double W_bubble_initial_dim = 18.0*1.0e-3;
 ...
 double Downstream_flux_backup = -0.2;

 double Prescribed_flux = -0.5;
 ...
 double Pre_stress = 40.0e3; //40.0e3;
 ...
 double Pressure_gradient_downstream_backup=0.6;
...
public:
 ...
 void actions_before_adapt()
  {
   oomph_info << "Tail node: "
              <<Problem_Parameter::Tail_node_pt->x(0)<<", "
              <<Problem_Parameter::Tail_node_pt->x(1)<<", "
              <<Problem_Parameter::Tail_node_pt->value(4)<<", "
              <<Problem_Parameter::Tail_node_pt->value(0)<<std::endl;
 ...
 void actions_after_adapt()
  {   
   ...
   oomph_info << "Flux behind bubble top: "
              << flux_behind_bubble_top<<std::endl;
 ...
 void apply_boundary_conditions()
  {
   // Enforce normal contact between the tail node and the boundary
   Problem_Parameter::Tail_node_pt->pin(6);
   Problem_Parameter::Tail_node_pt->set_value(6,0.0);

   Problem_Parameter::Tip_node_pt->pin(7);
   Problem_Parameter::Tip_node_pt->set_value(7,0.0);
     ...
     for (unsigned inod = 0; inod < num_nod; inod++)
      {
       ...
       if(ibound == Symmetry_boundary || 
          ibound == Outflow_boundary ||
          ibound == Top_boundary ||
          ibound == Top_inflow_boundary ||
          ibound == Actual_inflow_boundary )
        {        
         // Pin all FvK displacement values on top boundary
         if(ibound == Top_boundary)
          { 
           // Displacement
           nod_pt->pin(0);
         ...
         // Pin normal in-plane FvK displacement values on symmetry boundary
         if(ibound == Symmetry_boundary)
          { 
           // Displacement
           nod_pt->pin(3);
   ...
   for(unsigned inod=0; inod<num_nod; inod++)
    {
     ...
     if(nod_pt->is_on_boundary(Top_inflow_boundary))
      {
       Problem_Parameter::Tail_node_pt = nod_pt;
      }
    }
...
private:
 ...
 // Flag that indicates whether we prescribe the downstream flux
 bool Prescribe_flux;
   ...
   for(unsigned inod=0; inod<num_nod; inod++)
    {
     ...
     if(nod_pt->is_on_boundary(Symmetry_boundary))
      {
...
UnstructuredHSFvKProblem<ELEMENT>::UnstructuredHSFvKProblem()
 : Counter(0), Mesh_as_geom_object_pt(0), Constraint_mesh_pt(0), 
   ...
   Backed_up_surface_mesh_pt(0), Update_dependent_parameters(false)
{

 //enable_globally_convergent_newton_method();
 //Max_newton_iterations = 250;
 ...
 this->Problem::Max_residuals = 1.0e2; // 1.0e20;
 ...
 // Outer boundary: 5 distinct boundaries
 TriangleMeshClosedCurve* outer_boundary_pt = 0;
 Vector<TriangleMeshCurveSection*> outer_polyline_boundary_pt(5);

 // Segment vertex boundaries
 Vector<Vector<double> > bound_vertex(3);
 ...
 double unit_zeta = -0.5*MathematicalConstants::Pi/double(nsegment);
 Vector<Vector<double> > bound_hole(nsegment + 2);
 ...
 //First part
 for(unsigned ipoint = 0; ipoint < nsegment+1; ipoint++)
  {
...
int main(int argc, char **argv)
{
 ...
 if (CommandLineArgs::command_line_flag_has_been_set("--restart_file"))
  {
   Problem_Parameter::Ca_inv = 1.0/Problem_Parameter::Ca;
   ...
   Problem_Parameter::Target_aspect_ratio = Problem_Parameter::Aspect_ratio;
   Problem_Parameter::Aspect_ratio = 1.0/30.0;
   
   Problem_Parameter::Target_q = Problem_Parameter::Q;
   Problem_Parameter::Q = 15301.9;

   problem.steady_newton_solve(3);
   problem.doc_solution();
   problem.set_tip_node_pt();

   {
    Problem_Parameter::FSI_continuation = true;
    Problem_Parameter::Q_prev = Problem_Parameter::Q;
    Problem_Parameter::Max_increment = -1.0e3;
    ...
    //while(Problem_Parameter::FSI_continuation)
     {
      if( problem.calculate_minimum_angle_of_interface_elements() >
          Problem_Parameter::Min_angle_before_adapt)
       {
   ...
   //========================================================================
   {
    Problem_Parameter::Target_prescribed_flux=-0.4; //-0.18;
    ...
    while(Problem_Parameter::Prescribed_flux > 
          Problem_Parameter::Target_prescribed_flux)
     {
      ...
      if( problem.calculate_minimum_angle_of_interface_elements() >
          Problem_Parameter::Min_angle_before_adapt || count < 2)
       {
        actual_increment = problem.arc_length_step_solve(
   ...
   //========================================================================
   {
    Problem_Parameter::Target_prescribed_flux=-0.6; //-0.18;
    Problem_Parameter::Flux_continuation = true;
    Problem_Parameter::Prescribed_flux_prev = 
     Problem_Parameter::Prescribed_flux;
    Problem_Parameter::Max_increment = -2.5e-3;
 ...
 // No restart
 else
  {
   ...
   {
    Problem_Parameter::Target_U_bubble_tip = 1.0e-1; //3.524e-2; //1.0e-1; //3.12e-2;
    ...
    double desired_increment=1.0e-1; // minus: decrease U_tip, plus: increase
    double actual_increment=desired_increment;
    int count=0;
   ...
   //========================================================================
   Problem_Parameter::Target_prescribed_flux = -0.18; // -0.5; //-0.475;
   Problem_Parameter::Displacement_continuation = true;
   Problem_Parameter::Prescribed_displacement_prev = 
    Problem_Parameter::Prescribed_displacement;
   Problem_Parameter::Target_prescribed_displacement = 0.0;
   Problem_Parameter::Max_increment = -1.0; //-2.5e-3;
   {
    double desired_increment=1.0e-1;
    double actual_increment=desired_increment;
    int count=0;
   ...
   //========================================================================
   {
    Problem_Parameter::Target_prescribed_flux=-0.18;
    Problem_Parameter::Flux_continuation = true;
    Problem_Parameter::Prescribed_flux_prev = 
     Problem_Parameter::Prescribed_flux;
    Problem_Parameter::Max_increment = 2.5e-3;
   ...
   {
    Problem_Parameter::Target_U_bubble_tip = 2.0e0; //1.0e-6; //1.0e-1; //2.0e0;
    Problem_Parameter::Bubble_speed_continuation = true;
    Problem_Parameter::U_bubble_tip_prev = Problem_Parameter::U_bubble_tip;
