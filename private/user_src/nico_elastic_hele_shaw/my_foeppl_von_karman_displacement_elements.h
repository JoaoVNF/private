...
#ifndef OOMPH_MY_FOEPPLVONKARMAN_DISPLACEMENT_ELEMENTS_HEADER
#define OOMPH_MY_FOEPPLVONKARMAN_DISPLACEMENT_ELEMENTS_HEADER
...
public:
 ...
 typedef void (*FoepplvonKarmanPressureFctPt)(
   const Vector<double>& x, const double& w, double& f);
 ...
 /// \short Function pointer to pre-stress function -- spatially constant
 typedef void (*FoepplvonKarmanPreStressFctPt)(DenseMatrix<double>& sigma_0);
 ...
 FoepplvonKarmanDisplacementEquations() 
  : Pressure_fct_pt(0), Traction_fct_pt(0), Pre_stress_fct_pt(0)
  {
   //Set default
 ...
 FoepplvonKarmanPreStressFctPt& pre_stress_fct_pt() {return Pre_stress_fct_pt;}
 
 /// Access function: Pointer to pre-stress function. Const version
 FoepplvonKarmanPreStressFctPt pre_stress_fct_pt() const
 {return Pre_stress_fct_pt;}
 ...
 /// \short Get (spatially constant) pre-stress term.
 inline virtual void get_pre_stress_fvk(DenseMatrix<double>& sigma_0) const
 {
  //If no pre-stress function has been set, return zero
  if(Pre_stress_fct_pt==0)
   {
    sigma_0(0,0) = 0.0;
    sigma_0(1,1) = 0.0;
    sigma_0(0,1) = 0.0;
 ...
 /// Get gradient of deflection: gradient[i] = dw/dx_i
                 ...
                 const Vector<double> &interpolated_duydx)
  {
   ...
   sigma(0,0) = sigma_0(0,0) + 1.0/(1.0-_nu*_nu) * (e_xx + _nu*e_yy);
   
   // sigma_yy // obacht
   //sigma(1,1) = (e_yy + _nu*e_xx);
   sigma(1,1) = sigma_0(1,1) + 1.0/(1.0-_nu*_nu) * (e_yy + _nu*e_xx);
   
   // sigma_xy = sigma_yx // obacht
   //sigma(0,1) = sigma(1,0) = e_xy*(1.0 - _nu);
   sigma(0,1) = sigma(1,0) = sigma_0(0,1) + 1.0/(1.0+_nu) * e_xy;
     ...
     for(unsigned l=0;l<n_node;l++)
      {
       ...
       //Add contributions from current node/shape function // obacht
...
protected:
  ...
  /// Pointer to pre-stress function:
  FoepplvonKarmanPreStressFctPt Pre_stress_fct_pt;
