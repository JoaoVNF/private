...
//LIC// 
//LIC//====================================================================
//Non-inline functions for my FoepplvonKarman elements

#include "my_foeppl_von_karman_elements.h"
 ...
 for(unsigned ipt=0;ipt<n_intpt;ipt++)
  {
   ...
   //-------------------
   double pressure;
   get_pressure_fvk(ipt,interpolated_x,interpolated_w,pressure);

   double airy_forcing;
   get_airy_forcing_fvk(ipt,interpolated_x,airy_forcing);

   // Get pre-stress // obacht
   DenseMatrix<double> sigma_0(2,2,0.0);
   get_pre_stress_fvk(sigma_0);
     ...
     if(local_eqn >= 0)
      {
       ...
       if(!Linear_bending_model)
       {
        ...
        //   *test(l)*W;
        // obacht pre-stress terms added
        residuals[local_eqn] +=
           eta()*
         (interpolated_continuous_d2wdx2*
          (sigma_0(0,0)+interpolated_continuous_d2phidy2)
          + interpolated_continuous_d2wdy2*
          (sigma_0(1,1)+interpolated_continuous_d2phidx2)
          - 2.0*interpolated_continuous_d2wdxdy*
          (sigma_0(0,1)+interpolated_continuous_d2phidxdy))
         *test(l)*W;
       }
      }
