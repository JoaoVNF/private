...
#ifndef OOMPH_CONSTITUTIVE_MODEL_HEADER
#define OOMPH_CONSTITUTIVE_MODEL_HEADER
...
//===================================================================
/// A GeneralisedNewtonianConstitutiveEquation class 
/// defining a Hybrid fluid (mix of HB and Sisko)
/// using Tanner and Milthorpe's (1983) regularisation
/// with a smooth transition using a cubic (for n < 1)
//==================================================================
template<unsigned DIM>
 class HybridTanMilRegWithBlendingConstitutiveEquation : 
 public GeneralisedNewtonianConstitutiveEquation<DIM>
{
  private:
 ...
 /// "Cutoff regularised" Hybrid constitutive equation
 HybridTanMilRegWithBlendingConstitutiveEquation
  (double* yield_stress_pt, double* alpha_pt, double* flow_index_pt,  
   double* critical_second_invariant_pt) : 
  GeneralisedNewtonianConstitutiveEquation<DIM>(), 
   Yield_stress_pt(yield_stress_pt), Alpha_pt(alpha_pt), 
   ...
   Critical_second_invariant_pt(critical_second_invariant_pt)
   {
    ...
    oomph_info << "HybridTanMilRegWithBlendingConstitutiveEquation: "
               << " zero shear viscosity = " << zero_shear_viscosity
               << std::endl;
  ...
  double calculate_cutoff_viscosity()
  {
   // Calculate the Newtonian cutoff viscosity from the constitutive
   // equation and the cutoff value of the second invariant
   return 1.0+(*Yield_stress_pt)/(2.0*sqrt((*Critical_second_invariant_pt)))+
  ...
  void calculate_fitting_parameters_of_cubic(double& a, double& b)
  {
   ...
   a=1.0/(4.0*pow((*Critical_second_invariant_pt),3.0)*
          pow((*Critical_second_invariant_pt),5.0/2.0))*
    ((*Alpha_pt)*pow(2.0,(*Flow_index_pt))*((*Flow_index_pt)-1.0)*
     pow((*Critical_second_invariant_pt),2.0)*
     pow((*Critical_second_invariant_pt),(*Flow_index_pt)/2.0)-
     (*Alpha_pt)*pow(2.0,(*Flow_index_pt)+2.0)*
     pow((*Critical_second_invariant_pt),(*Flow_index_pt)/2.0+2.0)+
     8.0*pow((*Critical_second_invariant_pt),5.0/2.0)*
     (cut_off_viscosity+epsilon-1.0)-
     4.0*(*Yield_stress_pt)*pow((*Critical_second_invariant_pt),2.0)-
     (*Yield_stress_pt)*pow((*Critical_second_invariant_pt),2.0));
   ...
   b=-1.0/(4.0*pow((*Critical_second_invariant_pt),2.0)*
    ...
    ((*Alpha_pt)*pow(2.0,(*Flow_index_pt))*((*Flow_index_pt)-1.0)*
     pow((*Critical_second_invariant_pt),2.0)*
     pow((*Critical_second_invariant_pt),(*Flow_index_pt)/2.0)-
     3.0*(*Alpha_pt)*pow(2.0,(*Flow_index_pt)+1.0)*
     pow((*Critical_second_invariant_pt),(*Flow_index_pt)/2.0+2.0)+
     12.0*pow((*Critical_second_invariant_pt),5.0/2.0)*
     (cut_off_viscosity+epsilon-1.0)-
     6.0*(*Yield_stress_pt)*pow((*Critical_second_invariant_pt),2.0)-
     (*Yield_stress_pt)*pow((*Critical_second_invariant_pt),2.0));
  }
                   ...
                   second_invariant_of_rate_of_strain_tensor)
  {
   ...
   if(sign*second_invariant_of_rate_of_strain_tensor < 
   ...
   else
    {
     ...
     return 1.0+(*Yield_stress_pt)/(2.0*measure_of_rate_of_strain)+
      (*Alpha_pt)*pow((2.0*measure_of_rate_of_strain),
                      *Flow_index_pt-1.0);
    }
  }
...
//===================================================================
/// A GeneralisedNewtonianConstitutiveEquation class 
/// defining a Bingham fluid with pseudo-elasticity 
//==================================================================
template<unsigned DIM>
 class PseudoElasticBinghamBerEngRegConstitutiveEquation : 
 public GeneralisedNewtonianConstitutiveEquation<DIM>
{
  private:

 /// Bingham number
 double* Bingham_number_pt;

 /// Pseudo elastic number
 double* Pseudo_elastic_number_pt;

 /// Bercovier & Engelman regularisation parameter
 double* Regularisation_parameter_pt;

  public:

 PseudoElasticBinghamBerEngRegConstitutiveEquation
  (double* bingham_number_pt, double* pseudo_elastic_number_pt,
   double* regularisation_parameter_pt) : 
  GeneralisedNewtonianConstitutiveEquation<DIM>(), 
   Bingham_number_pt(bingham_number_pt), 
   Pseudo_elastic_number_pt(pseudo_elastic_number_pt),
   Regularisation_parameter_pt(regularisation_parameter_pt) {}
                   ...
                   second_invariant_of_rate_of_strain_tensor)
  {
   ...
   return 1.0+(*Bingham_number_pt)/
    (2.0*measure_of_rate_of_strain+
     exp(-2.0*measure_of_rate_of_strain*(*Pseudo_elastic_number_pt)));
  }
   ...
   (const double& second_invariant_of_rate_of_strain_tensor)
  {
   ...
   // obacht, hacky
   if(sign*second_invariant_of_rate_of_strain_tensor < 1.0e-15)
    {
     return -sign*second_invariant_of_rate_of_strain_tensor;
    }
   else
    {
     return (*Bingham_number_pt)*sign*(
      (*Pseudo_elastic_number_pt)*
      exp(2.0*(*Pseudo_elastic_number_pt)*
          measure_of_rate_of_strain)-
      exp(4.0*(*Pseudo_elastic_number_pt)*measure_of_rate_of_strain))/
      (measure_of_rate_of_strain*
       (2.0*measure_of_rate_of_strain*
        exp(2.0*(*Pseudo_elastic_number_pt)*measure_of_rate_of_strain)+1.0)*
       (2.0*measure_of_rate_of_strain*
        exp(2.0*(*Pseudo_elastic_number_pt)*measure_of_rate_of_strain)+1.0));
    }
  }
